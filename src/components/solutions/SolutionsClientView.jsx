...
/**
 * Universal Image Extractor for Solution Cards
 */
function getSolutionCardImageUrl(source) {
  ...
  if (source.asset?._ref || source._ref) {
    try {
      ...
      console.warn("Solution card image error:", e);
    }
  }
  if (source.image) return getSolutionCardImageUrl(source.image);
  if (source.heroImage) return getSolutionCardImageUrl(source.heroImage);
  if (source.mainImage) return getSolutionCardImageUrl(source.mainImage);

  return null;
}

export default function SolutionsClientView({ solutions = [] }) {
  const [activeCategory, setActiveCategory] = useState("all");

  const categories = ["all", ...new Set(solutions.map((sol) => sol.category).filter(Boolean))];

  const filteredSolutions = activeCategory === "all"
    ? solutions
    : solutions.filter((sol) => sol.category?.toLowerCase() === activeCategory.toLowerCase());
  ...
  return (
    <div className="w-full">
      ...
      {/* =========================================================
          STUNNING CARD GRID SHOWCASE WITH SQUARE IMAGES
      ========================================================= */}
      <motion.div 
        layout
      ...
      >
        <AnimatePresence>
          {filteredSolutions.map((solution, index) => {
            ...
            const slug = solution.slug?.current || solution.slug || "#";
            
            // Safely fetch image from any nested Sanity property for the card
            const imageUrl = getSolutionCardImageUrl(
              solution.hero?.image || 
              solution.image || 
              solution.heroImage || 
              solution.imageUrl || 
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5 rounded-full border border-[#C9A227]/30 bg-[#C9A227]/[0.08] px-3.5 py-1 text-[10px] font-bold tracking-wider text-[#9A7625] uppercase">
                      ...
                      <span>{solution.category || "Architecture"}</span>
                    </div>
                  </div>

                  {/* Title */}
                  <h3 className="mt-8 font-display text-2xl md:text-3xl tracking-tight text-[#111111] group-hover:text-[#9A7625] transition-colors">
                    {solution.title}
                  </h3>

                  {/* Short Description */}
                  {solution.shortDescription && (
                    <p className="mt-4 text-sm md:text-base leading-relaxed text-[#111111]/70">
                      {solution.shortDescription}
                    </p>
                  )}

                  {/* Integrations Stack Tags */}
                  {Array.isArray(solution.integrations) && solution.integrations.length > 0 && (
                    <div className="mt-6 pt-6 border-t border-[#111111]/5">
                      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#111111]/40 mb-3">Key Integrations:</p>
                      <div className="flex flex-wrap gap-2">
                        {solution.integrations.map((tech, tIdx) => (
                          <span
                            key={tIdx}
                            className="rounded-full border border-[#111111]/10 bg-[#FAF7F2] px-3 py-1 text-[11px] font-semibold text-[#111111]/80"
                          >
                            {tech}
                          </span>
                        ))}
      ...
      {filteredSolutions.length === 0 && (
        <div className="py-20 text-center">
          <p className="text-base text-[#111111]/60 font-medium">No solutions found under this category.</p>
        </div>
      )}
    </div>
  );
