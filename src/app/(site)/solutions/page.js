import { client } from "@/lib/sanity/client";
import { solutionsListQuery } from "@/lib/sanity/queries";
import SolutionsClientView from "@/components/solutions/SolutionsClientView";
import { Sparkles, ArrowUpRight } from "lucide-react";

export const revalidate = 60;

export const metadata = {
  title: "Solutions",
  description: "Composable solutions for migrations, headless commerce, ERP integrations, multi-store operations, and the workflows between them.",
};

const fallbackSolutions = [
  {
    _id: "fallback-migration",
    title: "Headless Migration",
    category: "Architecture",
    slug: { current: "headless-migration" },
    shortDescription: "Transition legacy monolithic setups to high-velocity decoupled architectures.",
    integrations: ["Shopify Plus", "Next.js", "Sanity CMS"]
  },
  {
    _id: "fallback-erp",
    title: "ERP & OMS Integration",
    category: "Operations",
    slug: { current: "erp-integration" },
    shortDescription: "Synchronize inventory, orders, and customer lifecycles across complex enterprise ERP systems.",
    integrations: ["NetSuite", "SAP", "Custom OMS"]
  },
  {
    _id: "fallback-multi",
    title: "Multi-Store Expansion",
    category: "Scale",
    slug: { current: "multi-store-expansion" },
    shortDescription: "Manage localized international storefronts, currencies, and catalogs seamlessly from a unified core.",
    integrations: ["Shopify Markets", "Global Pay", "Localization Engine"]
  }
];

export default async function SolutionsPage() {
  let solutions = [];
  try {
    solutions = await client.fetch(solutionsListQuery);
  } catch (error) {
    console.warn("Failed to fetch solutions from Sanity, using fallback.", error);
  }

  const displaySolutions = Array.isArray(solutions) && solutions.length > 0 ? solutions : fallbackSolutions;
  const isSanityData = Array.isArray(solutions) && solutions.length > 0;

  return (
    <main className="relative min-h-screen overflow-hidden bg-[#FAF7F2] text-[#111111]">
      
      {/* =====================================================
          BACKGROUND DOTTED MATRIX + PASTEL AMBIENT GLOWS
      ===================================================== */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -left-40 h-[650px] w-[650px] rounded-full bg-purple-200/40 blur-[140px]" />
        <div className="absolute top-1/3 -right-45 h-[700px] w-[700px] rounded-full bg-amber-200/40 blur-[150px]" />
        <div className="absolute -bottom-40 left-1/4 h-[600px] w-[600px] rounded-full bg-emerald-200/30 blur-[130px]" />

        <div 
          className="absolute inset-0 opacity-[0.45]"
          style={{
            backgroundImage: "radial-gradient(circle, rgba(201,162,39,0.55) 1.5px, transparent 1.5px)",
            backgroundSize: "36px 36px",
          }}
        />
      </div>

      {/* =====================================================
          REDESIGNED HERO SECTION
      ===================================================== */}
      <section className="relative z-10 px-6 pt-28 pb-20 md:pt-36 md:pb-28">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col items-start max-w-3xl">
            
            {/* Eyebrow Badge */}
            <div className="inline-flex items-center gap-2 rounded-full border border-[#C9A227]/40 bg-white/70 px-4.5 py-1.5 shadow-sm backdrop-blur-md">
              <Sparkles className="h-4 w-4 text-[#C9A227]" />
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#9A7625]">
                Composable Solutions
              </span>
            </div>

            {/* Main Headline */}
            <h1 className="mt-6 font-display text-5xl leading-[1.02] tracking-tight md:text-7xl lg:text-8xl text-[#111111]">
              Solve the system, not just the symptom.
            </h1>

            {/* Description */}
            <p className="mt-7 text-base leading-relaxed text-[#111111]/70 md:text-xl">
              Composable solutions for migrations, headless commerce, ERP integrations, multi-store operations, and the workflows between them.
            </p>
          </div>
        </div>
      </section>

      {/* =====================================================
          SOLUTIONS CLIENT VIEW (Cards & Filter)
      ===================================================== */}
      <section className="relative z-10 px-6 pb-32">
        <div className="mx-auto max-w-7xl">
          <SolutionsClientView
            solutions={displaySolutions}
            hasRealData={isSanityData}
          />
        </div>
      </section>

      {/* =====================================================
          BOTTOM CTA BANNER
      ===================================================== */}
      <section className="relative z-10 border-t border-[#111111]/10 bg-white/60 px-6 py-24 backdrop-blur-xl md:py-32">
        <div className="mx-auto max-w-5xl text-center">
          <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#9A7625]">
            Engineering Advisory
          </p>
          <h2 className="mt-4 font-display text-4xl tracking-tight text-[#111111] md:text-6xl">
            Ready to compose your technical blueprint?
          </h2>
          <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-[#111111]/70">
            Let&apos;s map out your infrastructure requirements and build a robust foundation for your enterprise scale.
          </p>

          <div className="mt-10 flex justify-center">
            <a
              href="/contact"
              className="inline-flex items-center gap-2 rounded-full bg-[#C9A227] px-8 py-4 text-sm font-semibold text-[#111111] transition-all hover:opacity-90 hover:shadow-xl shadow-[0_10px_30px_rgba(201,162,39,0.25)]"
            >
              Book a Consultation <ArrowUpRight className="h-4 w-4" />
            </a>
          </div>
        </div>
      </section>

    </main>
  );
}